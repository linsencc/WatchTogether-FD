...
function configureWebpack(webpackConfig, { env, paths }) {
    ...
    fileNames.forEach(fileName => {
        ...
        if (file.isDirectory()) {
            entries[fileName] = path.join(filePath, "index");
    ...
    function mkMiniCssExtractPlugin() {
    ...
    const defaultminiCssExtractPluginIndex = webpackConfig.plugins.findIndex(plugin => plugin instanceof MiniCssExtractPlugin);
    webpackConfig.plugins.splice(defaultminiCssExtractPluginIndex, 1, mkMiniCssExtractPlugin());


    //create-react-app默认用的是一个固定文件名，不适合多入口！改为按入口名生成输出文件名
    // if (isEnvDevelopment)
    webpackConfig.output.filename = 'static/js/[name]/[name].bundle.js';
